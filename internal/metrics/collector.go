...
// ~/.buildpecker/config.json (via system.GetAllNodes), samples host usage once, and
...
func registerInstruments(m metric.Meter) error {
	cpu, err := m.Float64ObservableGauge("buildpecker_node_cpu_percent",
		metric.WithDescription("Current host CPU usage percent (0-100)."))
	if err != nil {
		return err
	}
	memUsed, err := m.Int64ObservableGauge("buildpecker_node_memory_used_bytes",
		metric.WithDescription("Resident memory currently in use, in bytes."),
		metric.WithUnit("By"))
	if err != nil {
		return err
	}
	memTotal, err := m.Int64ObservableGauge("buildpecker_node_memory_total_bytes",
		metric.WithDescription("Total physical memory on the host, in bytes."),
		metric.WithUnit("By"))
	if err != nil {
		return err
	}
	diskUsed, err := m.Int64ObservableGauge("buildpecker_node_disk_used_bytes",
		metric.WithDescription("Disk space used on the root filesystem, in bytes."),
		metric.WithUnit("By"))
	if err != nil {
		return err
	}
	diskTotal, err := m.Int64ObservableGauge("buildpecker_node_disk_total_bytes",
